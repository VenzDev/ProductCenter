...
- **Mailgun (notification service)**: needs your own test-mode keys (app.mailgun.com/app/sending/domains) — `MAILGUN_DOMAIN`/`MAILGUN_API_KEY`/`MAILGUN_SENDER` in `services/notification/.env` (copy from `.env.example`; docker-compose loads it via `env_file`). `POST /api/v1/notifications/test-email` with `{"to": "you@example.com"}` sends a hardcoded hello-world email via Mailgun and returns the message id.
