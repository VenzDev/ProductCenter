...
#    ExternalSecret w infrastructure/k8s/backend/templates/externalsecret.yaml — jeden
#    (backend-app-secrets) z creationPolicy: Owner (tworzy Secret), drugi
#    (backend-db-secret) z Merge (dopisuje swój klucz, nie przejmując własności) — więc
#    Secret powstaje sam przy pierwszym `helm install backend` (krok 8), nic tu nie
#    trzeba odpalać. Patrz błąd #14.
...
| 16 | Po naprawieniu #14(b)/#15, `backend-db-secret` (drugi ExternalSecret, `creationPolicy: Merge`) ma `Reason: SecretOwnedByOther, Message: unable to mutate secret backend-secrets: secret is owned by another ExternalSecret: backend-app-secrets` | `Merge` nie jest pomyślany do współdzielenia jednego Secretu przez dwa ExternalSecret — kontroler przypisuje Secretowi jednego właściciela (ten z `creationPolicy: Owner`) i odrzuca mutacje od innego ExternalSecret, nawet z `Merge` | Zamiast dwóch ExternalSecret (jeden na SSM, jeden na Secrets Manager) — jeden ExternalSecret z domyślnym `secretStoreRef` (SSM) i pojedynczym kluczem (`db-password`) nadpisującym store przez `data[].sourceRef.storeRef` (Secrets Manager); dwa `SecretStore` zostają (AWS provider obsługuje jeden `service` na store), ale tylko jeden `ExternalSecret` — jeden właściciel, brak konfliktu | `infrastructure/k8s/backend/templates/externalsecret.yaml` |
