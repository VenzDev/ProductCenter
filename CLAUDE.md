...
- **Stripe (checkout payment)**: needs your own test-mode keys (dashboard.stripe.com/test/apikeys) — `STRIPE_SECRET`/`STRIPE_WEBHOOK_SECRET` in `services/backend/.env`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` in a root `.env` (copy from `.env.example`; docker-compose loads it for `${VAR}` substitution). Receiving the webhook locally needs `stripe listen --forward-to localhost:8081/api/v1/stripe/webhook` — Stripe's cloud can't reach `localhost` directly. Test card: `4242 4242 4242 4242`, any future expiry, any CVC.
